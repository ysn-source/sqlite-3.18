...
int sqlite3VdbeCheckMemInvariants(Mem *p){
  ...
  ** That saves a few cycles in inner loops. */
  ...
  //assert( p->szMalloc==0|| p->szMalloc==sqlite3DbMallocSize(p->db,p->zMalloc) );
...
int sqlite3VdbeMemFinalize(Mem *pMem, FuncDef *pFunc){
  ...
  if( ALWAYS(pFunc && pFunc->xFinalize) ){
	...
	if ((pFunc->funcFlags & SQLITE_WIN_FUNC)==0) t.flags = MEM_Null;
    t.db = pMem->db;
    ctx.pOut = &t;
    ctx.pMem = pMem;
    ctx.pFunc = pFunc;
...
int sqlite3VdbeMemWinFinalize(Mem *pMem, FuncDef *pFunc, Mem *pOut) {
	int rc = SQLITE_OK;
	if (ALWAYS(pFunc && pFunc->xFinalize)) {
		sqlite3_context ctx;
		Mem t;
...
int sqlite3VdbeMemCopy(Mem *pTo, const Mem *pFrom){
  ...
  memcpy(pTo, pFrom, MEMCELLSIZE); //+sizeof(pFrom->z)
  pTo->flags &= ~MEM_Dyn;
