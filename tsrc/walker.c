...
static SQLITE_NOINLINE int walkExpr(Walker *pWalker, Expr *pExpr){
  ...
  /*
  if (ExprHasProperty(pExpr, EP_Win)) {
	  if (sqlite3ResolveExprListNames(pWalker->u.pNC, pExpr->pWinBy)) return WRC_Abort;
	  if (sqlite3ResolveExprListNames(pWalker->u.pNC, pExpr->pWinOrder)) return WRC_Abort;
  }*/
  return WRC_Continue;
}
