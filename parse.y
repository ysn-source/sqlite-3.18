...
%fallback ID
  ...
  ROLLBACK SAVEPOINT TEMP TRIGGER VACUUM VIEW WEBVIEW VIRTUAL WITH WITHOUT 
...
ccons ::= LUCIA flagname(Y).      {sqlite3AddLuciaFlags(pParse,Y);}

%type flagname {int}

flagname(A) ::= WEBUSR. { A = COLFLAG_WEBUID; }
flagname(A) ::= WEBGRP. { A = COLFLAG_WEBGID; }
flagname(A) ::= WEBPWD. { A = COLFLAG_WEBPWD; }
...
///////////////////// The CREATE WEBVIEW statement /////////////////////////////
//
cmd ::= createkw(X) temp(T) WEBVIEW ifnotexists(E) nm(Y) dbnm(Z) eidlist_opt(C)
          AS select(S). { // authkw(R)
  sqlite3CreateWebView(pParse, &X, &Y, &Z, C, S, T, E); //, R
}
cmd ::= DROP WEBVIEW ifexists(E) fullname(X). {
  sqlite3DropTable(pParse, X, 1, E);
}

//authkw(A) ::= AUTH LT webrole(k) RP. {A=K;}
//%type webrole {ExprList*}
//%destructor webrole {sqlite3ExprListDelete(pParse->db, $$);}
...
%endif SQLITE_OMIT_COMPOUND_SELECT
oneselect(A) ::= SELECT(S) distinct(D) selcollist(W) into(I) from(X) where_opt(Y)
                 groupby_opt(P) having_opt(Q) orderby_opt(Z) limit_opt(L). {
#if SELECTTRACE_ENABLED
  Token s = S; /*A-overwrites-S*/
#endif
  A = sqlite3SelectNew(pParse,W,I,X,Y,P,Q,Z,D,L.pLimit,L.pOffset);
...
// selintolist is a list of bound parameters where will be returned
// selcolist values


silp(A) ::= selintolist(A) COMMA. 
silp(A) ::= .  {A = 0;}
selintolist(A) ::= silp(A) VARIABLE(X). { 
  
  ExprSpan D;
  u32 n = X.n;    
  spanExpr(&D, pParse, TK_VARIABLE, X);    
  A = sqlite3VListAdd(pParse->db,A,D.pExpr->u.zToken,n,-1); 
}
into(A) ::= INTO selintolist(X). {A=X;}
into(A) ::= . { A = 0; }

%type selintolist {VList*}
%destructor selintolist {sqlite3VListDelete(pParse->db, $$);}
%type silp {VList*}
%destructor selintolist {sqlite3VListDelete(pParse->db, $$);}
%type into {VList*}
%destructor into {sqlite3VListDelete(pParse->db, $$);}
...
%ifndef SQLITE_OMIT_SUBQUERY
  ...
  seltablist(A) ::= stl_prefix(A) LP seltablist(F) RP
    ...
    if( A==0 && Z.n==0 && N==0 && U==0 ){
      ...
      pSubquery = sqlite3SelectNew(pParse,0,0,F,0,0,0,0,SF_NestedFrom,0,0);
...
expr(A) ::= id(X) LP distinct(D) exprlist(Y) RP OVER(E) LP  elist_partition(P) elist_orderby(O) RP. {
  if( Y && Y->nExpr>pParse->db->aLimit[SQLITE_LIMIT_FUNCTION_ARG] ){
    sqlite3ErrorMsg(pParse, "too many arguments on function %T", &X);
  }
  A.pExpr = sqlite3ExprFunction(pParse, Y, &X);
  ...
  A.pExpr->flags |= EP_Win;
  A.pExpr->pWinBy=P;
  A.pExpr->pWinOrder=O;
}
elist_partition(A) ::= . {A=0;}
elist_partition(A) ::= PARTITION BY exprlist(X). { A=X;}
elist_orderby(A) ::= . {A=0;}
elist_orderby(A) ::= ORDER BY sortlist(X). { A=X;}
%type elist_partition {ExprList*}
%type elist_orderby {ExprList*}
%destructor elist_partition {sqlite3ExprListDelete(pParse->db, $$);}
%destructor elist_orderby {sqlite3ExprListDelete(pParse->db, $$);}
...
cmd ::= VARIABLE(X).               {sqlite3ShowVariable(pParse,&X);}
cmd ::= VARIABLE(X) EQ expr(V). { sqlite3DefineVariable(pParse,&X,V.pExpr); }
cmd ::= DEFINE VARIABLE(X) EQ expr(V). { sqlite3DefineVariable(pParse,&X,V.pExpr); }
...
cmd ::= createkw sp_decl(A) decl_param_list(P) IS BEGIN sp_cmd_list(S)  END(Z). { //AUTH role_list(R)
  Token all;
  all.z = A.z;
  all.n = (int)(Z.z - A.z) + Z.n;
  sqlite3FinishSP(pParse, S, &all);
  P=0; 
  //R=0;
}

sp_decl(A) ::= temp(T) WEBPROC ifnotexists(NOERR) nm(B) dbnm(Z). {
  sqlite3BeginSP(pParse, &B, &Z, T, NOERR);
  A = (Z.n==0?B:Z); /*A-overwrites-T*/
  ifindex=-1;
}


sp_cmd_list(A) ::= sp_cmd_list(A) sp_cmd(X) SEMI. {
  assert( A!=0 );
  A->pLast->pNext = X;
  A->pLast = X;
}
sp_cmd_list(A) ::= sp_cmd(A) SEMI. { 
  assert( A!=0 );
  A->pLast = A;
}

sp_cmd_list(A) ::= sp_cmd_list(A) sp_cmd_nosemi(X). {  assert( A!=0 );  A->pLast->pNext = X;  A->pLast = X; }
sp_cmd_list(A) ::= sp_cmd_nosemi(A). { assert( A!=0 ); A->pLast = A; }



pl(A) ::= . {A=0;}
pl(A) ::= paramlist(A) COMMA.
paramlist(A) ::=  pl(A) VARIABLE(W) typetoken(T) dflt.  {sqlite3AddParam(pParse,&W,&T);}
dflt ::= . 
dflt ::= DEFAULT expr(X). { sqlite3AddParamDflt(pParse,&X);}
decl_param_list(A) ::= LP paramlist(X) RP. {A=X;}
decl_param_list(A) ::= . {A=0;}

//rl(A) ::= . {A=0;}
//rl(A) ::= role_list(A) COMMA.
//role_list(A) ::= rl(A) expr(R). {sqlite3SPAddRole(pParse,&R); }





cmd ::= DROP WEBPROC ifexists(NOERR) fullname(X). { sqlite3DropSP(pParse,X,NOERR);}
cmd ::= EXEC fullname(X) exec_param_list(Y) onconf(A). { sqlite3ExecSP(pParse,X,Y,A);}
//cmd ::= EXECUTE fullname(X) exec_param_list(Y) onconf(A). { sqlite3ExecSP(pParse,X,Y,A);}

eparaml(A) ::= .  {A=0;}
eparaml(A) ::= eparamlist(A) COMMA.
eparamlist(A) ::=  eparaml(A) expr(X). { A = sqlite3ExprListAppend(pParse, A, X.pExpr);}
//eparamlist(A) ::= . { A=0;}

exec_param_list(A) ::= LP eparamlist(X) RP. {A=X;}
exec_param_list(A) ::= . {A=0;}
...
//sp_cmd(A) ::= . {A=0;}
// INSERT
sp_cmd(A) ::= insert_cmd(R) INTO trnm(X) idlist_opt(F) select(S). {A = sqlite3SPInsertStep(pParse->db, &X, F, S, R);/*A-overwrites-R*/}
// UPDATE 
sp_cmd(A) ::= UPDATE orconf(R) trnm(X) tridxby SET setlist(Y) where_opt(Z).    {A = sqlite3SPUpdateStep(pParse->db, &X, Y, Z, R);}
// DELETE
sp_cmd(A) ::= DELETE FROM trnm(X) tridxby where_opt(Y).  {A = sqlite3SPDeleteStep(pParse->db, &X, Y);}
// SELECT
sp_cmd(A) ::= select(X).   {A = sqlite3SPSelectStep(pParse->db, X); /*A-overwrites-X*/}

// BIND
sp_cmd(A) ::= DEFINE VARIABLE(X) EQ expr(V). {
	ExprSpan D;
	spanExpr(&D, pParse, TK_VARIABLE, X);
	A = sqlite3SPDefineStep(pParse->db,D.pExpr,X.n,V.pExpr);
}
sp_cmd(A) ::= VARIABLE(X) EQ expr(V). {
	ExprSpan D;
	spanExpr(&D, pParse, TK_VARIABLE, X);
	A = sqlite3SPDefineStep(pParse->db,D.pExpr,X.n,V.pExpr);
}
...
sp_cmd_nosemi(A) ::= IF expr(X) THEN.  { 
A = sqlite3SPIfStep(pParse->db,X.pExpr,TK_IF,++ifindex);
}
sp_cmd_nosemi(A) ::= ELSE. {
A = sqlite3SPIfStep(pParse->db,0,TK_ELSE,ifindex);
}
sp_cmd_nosemi(A) ::= ELSIF expr(X) THEN. {
A = sqlite3SPIfStep(pParse->db,X.pExpr,TK_ELSE,ifindex);
}
sp_cmd(A) ::= END IF. {   
  A = sqlite3SPIfStep(pParse->db,0,TK_END,ifindex--);
}

// FOREACH
sp_cmd_nosemi(A) ::= FOREACH LP select(X) RP LOOP.   {A = sqlite3SPForEachStep(pParse->db, X); /*A-overwrites-X*/}

sp_cmd(A) ::= END LOOP.   {A = sqlite3SPEndForEachStep(pParse->db); /*A-overwrites-X*/}




%type sp_cmd {SPStep*}
%destructor sp_cmd {sqlite3DeleteSPStep(pParse->db, $$);}

%type sp_cmd_nosemi {SPStep*}
%destructor sp_cmd_nosemi {sqlite3DeleteSPStep(pParse->db, $$);}


%type sp_cmd_list {SPStep*}
%destructor sp_cmd_list {sqlite3DeleteSPStep(pParse->db, $$);}


%type paramlist {Param*}
%destructor paramlist {sqlite3DeleteParam(pParse->db, $$);}
%type decl_param_list {Param*}
%destructor decl_param_list {sqlite3DeleteParam(pParse->db, $$);}
%type pl {Param*}
%destructor pl {sqlite3DeleteParam(pParse->db, $$);}
%type param {Param*}
%destructor param {sqlite3DeleteParam(pParse->db, $$);}
%type role_list {Role*}
%destructor role_list{sqlite3DeleteRole(pParse->db,$$);}
%type rl {Role*}
%destructor rl{sqlite3DeleteRole(pParse->db,$$);}


%type eparamlist {ExprList*}
%destructor eparamlist {sqlite3ExprListDelete(pParse->db, $$);}
%type eparaml {ExprList*}
%destructor eparaml {sqlite3ExprListDelete(pParse->db, $$);}
%type exec_param_list {ExprList*}
%destructor exec_param_list {sqlite3ExprListDelete(pParse->db, $$);}
%type eparam {Expr*}
%destructor eparam {sqlite3ExprDelete(pParse->db, $$);}


%endif  !SQLITE_OMIT_SP
